...
public class Tree {
  ...
  public void addChild(List<Tree> child) {
    this.childs = (List<Tree>) child;
  }
  /**
   * 置空树
   */
  ...
  public String toString() {
    ...
    toString.append(token.getKey());
    if(!isLeaf()){  //叶节点
    	if(token.getLine() != -1)
    		toString.append("("+token.getLine()+")");
      return String.valueOf(toString);
    }
    if(token.getValue()!=null){
      toString.append(" :").append(token.getValue());
    }
    if(token.getLine() != -1)
		toString.append("("+token.getLine()+")");
  return String.valueOf(toString);
